...
describe("IMDB Search Title", () => {
  ...
  it("search for movies by title", async () => {
    ...
    for (let i = 0; i < movieTitlesCSS.length; i++) {
      ...
      if (title.includes("Mark")) newItems.push(title);
    }
    ...
    console.log(newItems.length, moviesOnPage, newItems);
...
describe("IMDB Search Date", () => {
  it("search movies by date and verify results", async () => {
    ...
    const newItems = [];
    const movieList = await $$(".lister-item-year");

    // Matches the first number in the string

    for (let i = 0; i < movieList.length; i++) {
      const isTVSeries = await movieList[i].getText();
...
describe("IMDB Search Rating", () => {
  it("search movies by rating and verify results", async () => {
    // To  be developed
  }); //inline-block ratings-imdb-rating
});
describe("IMDB Search Genres", () => {
  it("search movies by genres and verify results", async () => {
    // To  be developed
  }); //inline-block ratings-imdb-rating
});
